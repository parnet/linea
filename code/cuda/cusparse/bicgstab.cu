...
int gpu_BiCGStab(cublasHandle_t       cublasHandle,
    ...
    //--------------------------------------------------------------------------
    // ### 2 ### repeat until convergence based on max iterations relative residual
...
int test_main(CRS_Matrix mat) {
    ...
    int     m           = mat._num_rows;
    std::cout << "m            =" << mat._num_rows << std::endl;
    std::cout << "mat._num_rows=" << mat._num_rows << std::endl;
    int nnz = mat._row_ptr[mat._num_rows];
    std::cout << nnz << std::endl;
    auto h_X = (double*)malloc(m * sizeof(double));
    ...
    CUDA_CRS_Matrix cu_mat = mat;
    cudaStreamSynchronize(cu_mat.stream);
    ...
    cusparseSpMatDescr_t matA, matM_lower, matM_upper;
    cusparseMatDescr_t   matLU;
    int*                 d_M_rows      = cu_mat.d_row_ptr;
    int*                 d_M_columns   = cu_mat.d_col_idx;
    cusparseFillMode_t   fill_lower    = CUSPARSE_FILL_MODE_LOWER;
    cusparseDiagType_t   diag_unit     = CUSPARSE_DIAG_TYPE_UNIT;
    cusparseFillMode_t   fill_upper    = CUSPARSE_FILL_MODE_UPPER;
    cusparseDiagType_t   diag_non_unit = CUSPARSE_DIAG_TYPE_NON_UNIT;
    // A
    CHECK_CUSPARSE( cusparseCreateCsr(&matA, m, m, nnz, cu_mat.d_row_ptr, cu_mat.d_col_idx, cu_mat.data, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, baseIdx, CUDA_R_64F) )
    CHECK_CUSPARSE( cusparseCreateCsr(&matM_lower, m, m, nnz, d_M_rows, d_M_columns, d_M_values, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, baseIdx, CUDA_R_64F) )
    CHECK_CUSPARSE( cusparseCreateCsr(&matM_upper, m, m, nnz, d_M_rows, d_M_columns, d_M_values, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, baseIdx, CUDA_R_64F) )
    ...
    CHECK_CUSPARSE( cusparseSpMatSetAttribute(matM_upper, CUSPARSE_SPMAT_DIAG_TYPE, &diag_non_unit, sizeof(diag_non_unit)) )
    //--------------------------------------------------------------------------
    // ### Preparation ### b = A * X
    ...
    void*  d_bufferMV;
    double beta = 0.0;
    CHECK_CUSPARSE( cusparseSpMV_bufferSize( cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, d_X.vec, &beta, d_B.vec, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSizeMV) )
    CHECK_CUDA( cudaMalloc(&d_bufferMV, bufferSizeMV) )

    CHECK_CUSPARSE( cusparseSpMV( cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, d_X.vec, &beta, d_B.vec, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, d_bufferMV) )
    // X0 = 0
    CHECK_CUDA( cudaMemset(d_X.ptr, 0x0, m * sizeof(double)) )
    //--------------------------------------------------------------------------
    // Perform Incomplete-LU factorization of A (csrilu0) -> M_lower, M_upper
    ...
    CHECK_CUSPARSE( cusparseSetMatIndexBase(matLU, baseIdx) )
    CHECK_CUSPARSE( cusparseCreateCsrilu02Info(&infoM) )
    CHECK_CUSPARSE( cusparseDcsrilu02_bufferSize( cusparseHandle, m, nnz, matLU, d_M_values, cu_mat.d_row_ptr, cu_mat.d_col_idx, infoM, &bufferSizeLU) )
    CHECK_CUDA( cudaMalloc(&d_bufferLU, bufferSizeLU) )
    CHECK_CUSPARSE( cusparseDcsrilu02_analysis( cusparseHandle, m, nnz, matLU, d_M_values, cu_mat.d_row_ptr, cu_mat.d_col_idx, infoM, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_bufferLU) )
    int structural_zero;
    CHECK_CUSPARSE( cusparseXcsrilu02_zeroPivot(cusparseHandle, infoM, &structural_zero) )
    // M = L * U
    CHECK_CUSPARSE( cusparseDcsrilu02( cusparseHandle, m, nnz, matLU, d_M_values, cu_mat.d_row_ptr, cu_mat.d_col_idx, infoM, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_bufferLU) )
    // Find numerical zero
    int numerical_zero;
    ...
    // ### Run BiCGStab computation ###
    printf("BiCGStab loop:\n");
    gpu_BiCGStab(cublasHandle, cusparseHandle, m, matA, matM_lower, matM_upper, d_B, d_X, d_R0, d_R, d_P, d_P_aux, d_S, d_S_aux, d_V, d_T, d_tmp, d_bufferMV, maxIterations, tolerance);
    //--------------------------------------------------------------------------
    // ### Free resources ###
    CHECK_CUSPARSE( cusparseDestroyDnVec(d_B.vec) )
    CHECK_CUSPARSE( cusparseDestroyDnVec(d_X.vec) )
